/* Roll a Hero — the campaign, for the Dungeon Master OS.
   ...
   v4 structure (2026-08-10, per Joby): scenes are FOLDERS that contain their
   components; NPCs and Monsters each have a top-level master folder holding
   every sheet, and scenes point at those sheets with [[wikilinks]].

   Act One (folder)
     └─ LASTLIGHT — the opening scene (folder)
          └─ the run sheet (scene doc; encounter + handouts land here next)
   NPCs — the cast (folder):     Wenna, Tobin, Finch, Hettie
   Monsters — the bestiary (folder): (tear-monster family arrives next push) */
window.DM_CAMPAIGN = {
  ...
  { "schema": 1, "id": "f-lastlight", "type": "folder",
    ...
    "parent": "act1", "order": 10, "rev": 1,
    "tags": ["act1", "session1"], "leadsTo": [], "fields": {},
    "body": "Session one, doubling as session zero. Everything needed to run the opening lives in this folder: the run sheet now, and next push the fairground encounter (snatchers, chasers, a far-off heavy) and the handouts (reason cards, the Lastlight program). Cast on stage: [[npc-wenna|Wenna Ash]], [[npc-finch|Finch]], [[npc-hettie|Hettie Sarn]] (seen, not met) — and by the morning after, [[npc-tobin|Tobin Rell]]." },

  { "schema": 1, "id": "sc-lastlight", "type": "scene",
    "title": "LASTLIGHT — the run sheet",
    "parent": "f-lastlight", "order": 10, "rev": 2,
  ...
  { "schema": 1, "id": "f-monsters", "type": "folder",
    ...
    "parent": null, "order": 30, "rev": 1,
    "tags": ["bestiary"], "leadsTo": [], "fields": {},
    "body": "Every campaign creature lives here — one sheet each, linked from the scenes and encounters that use it. First residents arrive with the next push: the Lastlight tear family (snatchers, chasers, and the far-off heavy). The SRD starter creatures live separately in the 📖 Look up library; sheets here are this campaign's own." },
