...
> - **NPC stat blocks + quick-generate (2026-07-09):** the NPC template has an *optional* combat block (`DOC_TYPES.npc.statBlock` = hp/ac/attack/special) rendered only once it has values, so non-combat NPCs stay clean. A **⚡ Stats** button in the doc header drops a Normal/Heroic/Legendary/Epic menu (`TIER_STATS`) that fills tier-appropriate stats (works on creatures too); Cowork fills the same keys. Once an NPC has HP it auto-fills in the roster (`hpFromDoc` is generalized).
...
>
> **Where to pick up next (Joby's call):** the "At the table" system AND the **story flow chart** are now built — the map is a full authoring surface (see the Story-map bullet above). Quick wins still queued in `BACKLOG.md`: **grow the SRD bestiary** (more starter monsters — just data in `bestiary.js`), and **sync roster HP when stats are generated** (today the roster snapshots HP at add-time, so a later quick-generate leaves the tracker blank). Map follow-ups if wanted: drag *card-to-card* to connect (today connecting is right-click → picker); edit an existing edge's kind/label in place; a mini-map for very large campaigns. Minor: the hero card's passive perception is 10+WIS baseline (the builder doesn't model skill proficiency).
>
