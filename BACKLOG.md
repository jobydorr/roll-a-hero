...
## ✅ Insert existing NPC / creature (raised by Joby 2026-08-10 — SHIPPED same day; reworked to FULL-SHEET EMBEDS after table feedback, dmos-ui `v34`)

**The want:** when choosing "new NPC" or "new creature" in the DM OS, a way to pick from the NPCs and creatures that already exist in the workspace instead of creating a duplicate sheet — **and see the full sheet in the folder, not a link** (Joby's correction to the v33 first cut, which inserted a `Cast:` wikilink line into the body).

**As built (v34 — "masters contain, scenes cast"):** every container's ＋ menu ends with **Existing NPC…** / **Existing creature…** (shown only when a sheet of that type exists somewhere; hidden at the root). Picking opens a searchable list of every sheet of that type wherever it is filed. Choosing one stores a `leadsTo` entry of kind `cast` on the container — structured data, not body text — and the feed then renders the member's **entire sheet, read-only, inside the container**: stat rows, ＋ To the table, body, an open-the-sheet button, and an ✕ that removes it from the cast without touching the sheet. The one true sheet stays in its master folder; edit it there and every place it is cast follows (the embed's reconcile key folds in the target's rev, and is placement-unique — `cast:host:target` — so a sheet can appear in a feed alongside its own embed without breaking the keyed reconcile). Cast refs are invisible to the story map and protected from all edge-editor operations. Dedup keys off cast entries only, so wikilinks in a container's prose no longer block the picker (the v33 bug Joby hit searching for Wenna). Verified end-to-end in a local DM OS: embeds, double-appearance, unlink→search→re-add roundtrip, map isolation, no console errors. Code: `renderCastEmbed`/`embedKey` + the entries pass in `PAINT.feed`, `cast-open`/`cast-pick`/`cast-unlink`/`cast-search:input` in `dmos-ui.js`; small `.doc-cast-embed` block in `dmos.css` (v25).

**⚠️ SIMPLIFICATION PASS (v36 — the one that matters; earlier versions overbuilt this).** Joby's rule, and the only rule: **add an NPC or monster anywhere and it looks the same everywhere; removing it from one place must never remove it from anywhere else.** So: a sheet added to a scene renders as the *identical card* it is at home — same fields, fully editable (edits patch the one sheet), same ＋ To the table / ⚡ Stats buttons, no dimming, no "cast" badge, no read-only variant. Tree rows for added members are literally the same rows as any other, drag handles included. The only difference anywhere is what **✕** means: on a sheet in its own home folder it still moves to trash (with a confirm that now *names every other place the sheet appears*), and on a sheet shown inside a container it was added to it just takes it out of that container. `renderCastEmbed` is deleted; `renderDocNode(d, depth, castHost)` is the single renderer. Verified: removing from a scene leaves the sheet in its folder, in the picker, and out of the trash; editing from the scene patches the real sheet.

**UX pass (v35 — superseded in part by v36 above):** (1) **cast members now appear in the sidebar tree** under their container, as dimmed badged rows with no drag handle — click one and the real sheet opens; a member that is also a physical child shows only its real row, and non-folder containers (an encounter with a cast) get a twist too. (2) **Each sheet renders at most once per feed** — a real node beats an embed, and the first cast placement beats later ones, so casting the same person in an act and in a scene inside it no longer doubles her up. (3) **The picker is now the cast manager**: members are always listed with a "✓ in this cast" badge instead of vanishing into an empty "everything is already here" box, clicking toggles in/out, and the dialog stays open for multi-add. Code: `treeCastMembers` in `treeNodeHTML`, the `wantIds`/`placed` dedupe in `PAINT.feed`, toggle `cast-pick` + always-listed `renderCastList`; `dmos.css` v26.
...
- ✅ **NPC stat blocks + quick-generate (2026-07-09).** The NPC template has an *optional* combat block (`DOC_TYPES.npc.statBlock` = hp/ac/attack/special), rendered only once it has values. A **⚡ Stats** button drops a **Normal / Heroic / Legendary / Epic** menu that fills tier-appropriate stats (works on creatures too). **Now RANDOMIZED (2026-07-09):** each tier is a set of ranges (`TIER_STATS` in `dmos-ui.js`: hp/ac/to-hit/damage-bonus/speed spans + a list of original special moves) rolled fresh on every generate, so two "Heroic" monsters aren't identical.
- ✅ **Creature-block polish (2026-07-09).** (a) A **📖 Look up** button on a creature page opens a search of the creature library and drops the chosen creature's stats straight into *that* block (`creature-fill`/`fill-pick`; auto-titles an "Untitled" one, fills notes, and — via `patch()` → `syncRosterHpToDoc` — updates the roster if it's on the table). (b) **Stats sit at the top of the info block now:** the NPC combat block renders *above* the looks/wants/voice/secret fields (divider moved underneath). (c) **Density pass** — trimmed the DM-OS padding throughout (doc/field/label/body spacing, tools, tree rows, feed) so pages read denser than the builder's airy wizard layout.
- ✅ **Roster HP sync on generate/edit (2026-07-09).** When a creature/NPC's page **HP** changes (a hand edit, or a ⚡ Stats generate) and that doc is on the initiative roster (a `kind:'doc'` entry referencing it), the tracker's max now updates to match: a combatant at full (or with no HP tracked yet) refills to the new max; a bloodied one keeps its current HP, only clamped down if the new max is lower. Non-HP edits (AC/speed/…) are untouched. Implemented as `syncRosterHpToDoc(id)` in `dmos-store.js`, called from `patch()` only when the partial touches `fields.hp`. (Fixes the reported "AC/movement update in the rail but HP doesn't" — the row chip tracked the add-time snapshot.)
- ✅ **Link from highlighted text (2026-07-09).** Highlight text in a document's **body** → a floating **🔗 Link** marker pops up at the selection → click it → a **pick-by-title** search of your documents → the highlighted text is wrapped as a `[[id|highlighted text]]` wikilink. Robust: the splice runs on the exact selection offsets captured at highlight time (`linkSel = {value,start,end}`), so it survives the editor blurring when the picker takes focus; the label is sanitised (`]`/`[` stripped). Because the Story map already reads `[[links]]` as "mentions" edges, a link made this way **appears on the map automatically**. Code: the *Link from a selection* block in `dmos-ui.js`; `.link-marker` styles in `dmos.css`; `mouseup`/`keyup` added to the shell delegation.
- ✅ **Story flow chart — now an authoring surface (2026-07-09).** A **Story map** in Tools → its **own full-viewport board** (the `#dmosMap` root, NOT a floating dialog), showing every non-folder story doc as a card, wired by the links already on the docs: **`leadsTo`** entries become solid, arrow-headed, labelled edges (coloured by kind — *leads to* green / *alternative* gold / *knows* blue), and **`[[wikilinks]]`** become thin dashed "mentions" edges (skipped when a stronger `leadsTo` already joins the pair). Auto-layout is **flow-directed swimlanes**: one horizontal band per Act (top-level folder), and within a band cards flow left→right by `leadsTo` depth with a barycentre row-sort so edges run *beside* cards, not through them.
  ...
  - **Drag-to-connect + edge editor + mini-map (2026-07-09).** Each card has a **○ connector handle** on its right edge — **drag it onto another card** to create a `leadsTo` (kind `then`, no label); a dashed temp edge follows the cursor and the target highlights. **Click any arrow** (its label pill, or the small **✎ nub** on a label-less link) → an **edge editor** to change the kind (Leads-to/Alternative/Knows), set/clear the label, or **Remove link**. A **mini-map** appears bottom-right whenever the board overflows the viewport — every card as a tiny type-coloured rect with a viewport frame; click/drag it to pan. Esc mid-drag cancels the gesture instead of closing the map.
  - **Navigation/UX:** **drag** a card to rearrange (position persists per-browser in `rollAHeroDmChart` via `getChart`/`setChartPos`/`clearChart`); **click** a card to jump to it in the feed; **drag the board** to pan; **zoom** with the toolbar −/Fit/+ or Ctrl/⌘-wheel; **Reset layout** snaps back to auto; **Esc**/**Done** closes. A dangling `leadsTo` shows a ⚠ on the card + a header count instead of a broken edge. Derived fresh on every (re)paint, so it can't drift from the story; its right-click menus/pickers/edge-editor live in `#dmosModal` and overlay the board (z: map 48 < modal 50). Files: the *Story flow map* section of `dmos-ui.js`, styles under *Story map* in `dmos.css`, the `#dmosMap` root in `dm.html`, the `flow` icon in `icons.js`.
...
- **In-app "DM's Assistant" (AI chat that builds the workspace)** 🟠 — *raised by Joby 2026-07-10; write-up only for now, don't build until he greenlights the approach.* The dream: a chat panel in the DM OS left sidebar where the DM brainstorms creatures / NPCs / world & story, and the assistant **actually creates and edits the cards** (not just talks).
  - **What's achievable vs not.** A literal 1:1 port of *this* Cowork/Claude Code interface **cannot** run inside the DM OS: that's an agent with a filesystem, `git`, and a shell (it edits `campaign.js` and pushes). A GitHub Pages page is a sandboxed browser — no file/git access, so that capability isn't portable. **What IS achievable:** a chat that calls the Anthropic API (`POST /v1/messages`, model `claude-opus-4-8`) with **tool-use tools mapped to the store's own functions** (`createDoc`/`patch`/`deleteDoc`/`createSection`/`rosterAdd`/`saveToBestiary`/`setChartPos`/`get`/`docs`/search). Drive the loop with the SDK Tool Runner or a manual `while stop_reason=="tool_use"` loop. That gives the "same editing capability" — it edits the DM's **documents**, live, the same way the hand tools do.
  - **Where content lands:** the **local workspace** (the localStorage overlay the DM's own edits use), NOT `campaign.js`. Better for private prep (never hits the public repo), but assistant-made content lives in that browser until exported/committed. (Contrast: Cowork authors into `campaign.js`.)
  - **Three real decisions (Joby's, because they cost money / change guarantees):** (1) **Backend/key** — a public static site can't embed an API key; either **bring-your-own-key** (DM pastes their Anthropic key, stored per-browser like the passcode; note direct browser calls need `anthropic-dangerous-direct-browser-access` and expose the key to that browser session) or a **tiny serverless proxy** (Cloudflare Worker/Vercel free tier) that holds the key and can cap usage — a departure from the app's no-server design. (2) **Cost** — pay-per-message to Anthropic; not free like the rest of the app. (3) **Local-first shift** — prompts/content leave the browser to the API, breaking the current "nothing leaves your browser unless you Share" guarantee (fine for a DM tool, but call it out).
  - **Recommended first slice (Joby leaned toward "just the write-up" 2026-07-10):** prototype the **chat panel UI + the tool/function layer against the store** with the model stubbed, so the shape and the create/edit/link/roster actions are provable before committing to a paid backend; wire the real API (BYO-key first) after. Keep it its own left-sidebar area, gated behind the passcode like everything else.
